...
def authenticate(code, subscription_id):
    payload = f'client_id={client_id}&client_secret={client_secret}&grant_type=authorization_code&scope=TPPOAuth2Security&code={code}'
    try:
        response = requests.request("POST", url, headers=headers, data=payload)
        if response.status_code == 200:
            return response.json()            
    except:
        raise Exception("Error with authorizing (step 3 of OAuth)")
    raise Exception("Error with authorizing (step 3 of OAuth)")
